...
This document describes the system architecture, major components, and responsibilities of each main layer within the NetTrace Alarm Analytics backend.

## Architecture Overview

The backend follows a practical one-way layered structure around Express request handling. The source code contains Express route modules, but these routes are HTTP adapters, not a separate backend layer. Request handling moves from the HTTP adapter into controllers, then services, repositories, and database clients.
...
Express route modules map HTTP verbs and URL paths to controllers, register validation middleware, and declare Swagger OpenAPI specs via JSDoc comments. They support request handling but are not counted as a standalone backend layer.

## Component Interactions

All component interactions are downstream in the current backend implementation. Request handling flows from the HTTP adapter to Controller to Service to Repository to the Databases. The result is returned back up the call stack to the Controller, which responds to the client. Direct communication between repositories and controllers, or services and route modules, is not used in the current source code.
